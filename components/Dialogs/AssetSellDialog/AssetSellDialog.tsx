...
const AssetSellDialog: React.FC = (): JSX.Element => {
  ...
  const [buttonText, setButtonText] = useState<string>("SELL ASSET");
  ...
  const sellAsset = async () => {
    ...
    if (price > 0) {
      ...
      try {
        ...
        console.error(error);
        dispatch(closeDialog());
        showWarning("Selling " + asset?.metadata?.name + " was aborted.");
      }
    } else {
      showError("Please enter a price for your chest!");
    }
  };
    ...
    >
      <div className="min-h-screen text-center flex justify-center items-center">
        ...
        <div className="inline-block w-full text-elementor-text-3 max-w-[500px] text-left align-middle rounded-sm transition-all transform bg-elementor-1 bg-gradient-to-b p-[2px] from-elementor-text-3 to-[#64a2bc] shadow-xl">
          <div className="bg-[#11131c] h-[325px] flex">
            <CloseDialogButton />
            <div className="w-full h-full flex justify-center items-center flex-col gap-2">
              ...
              <div className="relative inline-block text-left" ref={ref}>
                ...
                <div className="absolute right-0 mt-2 w-[140px] z-30 rounded-md bg-elementor-text-2">
                  <div
              ...
              {selected !== "Select currency" && (
                <>
                  ...
                  <button
                    disabled={version || disabled}
                    className="duration-300 z-10 text-md tracking-widest bg-[#3E6A86] px-2 py-1 rounded-sm font-display font-bold text-black text-center hover:bg-elementor-text-3 disabled:hover:text-black disabled:opacity-25 disabled:hover:bg-[#3E6A86] disabled:cursor-not-allowed"
                    onClick={!isConnectedToImx ? connectWalletToImx : sellAsset}
                  >
