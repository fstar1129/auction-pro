...
const PirateTransferDialog: React.FC = (): JSX.Element => {
  ...
  const checkTransferStatus = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_IMMUTABLE_X_URL}/v1/assets/${process.env.NEXT_PUBLIC_IMX_PIRATES_COLLECTION}/${pirate?.token_id}`
      );
      if (res.data?.user === user.keys.walletAddress) {
        ...
        showSuccess(
          "Pirate " + pirate?.metadata?.name + "was transfered successfully!"
        );
      }
    } catch (error) {
  ...
  const transferPirate = async () => {
    let startsWith = reciver.substring(0, 2);

    if (pirate?.status === "imx") {
      if (startsWith === "0x" && reciver.length === 42) {
        ...
        try {
          await link.transfer([
            {
              ...
              tokenId: pirate?.token_id,
              tokenAddress: `${process.env.NEXT_PUBLIC_IMX_PIRATES_COLLECTION}`,
              toAddress: reciver,
            },
          ]);
          ...
          dispatch(closeDialog());
          showWarning(
            "Transfering pirate " + pirate?.metadata?.name + " was aborted."
          );
        }
      } else {
        showError(
            ...
            <div className="w-full h-full flex justify-center items-center flex-col">
              ...
              <button
                ...
                className="duration-300 z-10 text-md tracking-widest bg-[#3E6A86] mt-2 px-2 py-1 rounded-sm font-display font-bold text-black text-center hover:bg-elementor-text-3 disabled:opacity-[0.25] disabled:hover:bg-[#3E6A86]
                  disabled:hover:opacity-[0.25] disabled:hover:cursor-not-allowed"
                onClick={
                  !isConnectedToImx ? connectWalletToImx : transferPirate
                }
              >
...
export default PirateTransferDialog;
