...
const ItemTransferDialog: React.FC = (): JSX.Element => {
  ...
  const checkTransferStatus = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_IMMUTABLE_X_URL}/v1/assets/${process.env.NEXT_PUBLIC_IMX_ITEMS_COLLECTION}/${item?.token_id}`
      );
      if (res.data?.user === user.keys.walletAddress) {
        ...
        showSuccess(
          "Item " + item?.metadata?.name + " was transfered successfully!"
        );
      }
    } catch (error) {
    ...
    if (item?.status === "imx") {
      if (startsWith === "0x" && reciver.length === 42) {
        ...
        try {
          ...
          dispatch(closeDialog());
          showWarning(
            "Transfering item " + item?.metadata?.name + " was aborted."
          );
        }
      } else {
        showError(
          ...
          <div className="bg-[#11131c] h-[200px]">
            <div className="w-full h-full flex justify-center items-center flex-col">
              ...
              <button
                ...
                onClick={!isConnectedToImx ? connectWalletToImx : transferItem}
              >
